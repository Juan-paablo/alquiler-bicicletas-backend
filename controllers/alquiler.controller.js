...
exports.alquilar = async (req, res) => {
    ...
    const { usuario, estacionSalida, bicicleta } = req.body;

    try {
    if (!usuario?.nombre || !usuario?.correo) {
      return res.status(400).json({ message: 'Faltan datos de usuario (nombre, correo).' });
    }
    ...
    const yaActivo = await alquilerModel.findOne({ 'usuario.correo': usuario.correo, activo: true }).lean();
    if (yaActivo) {
